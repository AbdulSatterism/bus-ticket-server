...
const createPurchesTicket = async (payload: TPurchase) => {
    ...
    if (!ticketExist) {
      throw new AppError(httpStatus.BAD_REQUEST, 'This ticket not available in database');
    }
    ...
    if (!userExist) {
      throw new AppError(httpStatus.BAD_REQUEST, 'This User not available in database');
    }
   ...
   await Bus.findByIdAndUpdate(busId,{ $inc: { availableSeats: -1 } },{new:true})
